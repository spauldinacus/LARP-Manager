...
export async function setSessionData(res, sessionData) {
  const encryptedSession = await sealData(sessionData, sessionOptions);
  
  res.setHeader('Set-Cookie', `${sessionOptions.cookieName}=${encryptedSession}; HttpOnly; Path=/; Max-Age=${sessionOptions.cookieOptions.maxAge}${sessionOptions.cookieOptions.secure ? '; Secure' : ''}; SameSite=Lax`);
}
...
export async function getSessionData(req) {
  try {
    ...
    return await unsealData(sessionCookie, sessionOptions);
  } catch (error) {
...
export function clearSession(res) {
  res.setHeader('Set-Cookie', `${sessionOptions.cookieName}=; HttpOnly; Path=/; Max-Age=0${sessionOptions.cookieOptions.secure ? '; Secure' : ''}; SameSite=Lax`);
}
